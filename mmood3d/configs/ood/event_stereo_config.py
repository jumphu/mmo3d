...
# Training configuration for ICCV 2025 Event Stereo reproduction
...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        # Voxelization and normalization configs would go here
    ),
    backbone=dict(
        type='EventStereoBackbone',
        in_channels=15, # 15 time bins for event voxel grid
        out_channels=32,
        num_blocks=3
    ),
    ...
    roi_head=dict(
        type='EventROIAlignmentHead',
        in_channels=128, # Compressed BEV channels
        roi_size=7,
        out_channels=128
    )
)
...
train_pipeline = [
    ...
    dict(type='CustomPack3DDetInputs', 
         keys=['event_l', 'event_r', 'cam2ego_l', 'cam2ego_r', 'cam_intrinsic'],
         meta_keys=['sample_idx', 'token', 'timestamp', 'pose'])
]
